...
public class KIESpringBusinessRulesTest extends AbstractTestNGSpringContextTests {
	...
	public void validateKieModuleMemoryFileSystem() {
		...
		// spongecell:core:0.0.1-SNAPSHOT to the repository
		// *************************************************
		KieSession kieSession1 = kieSessionHandler
				.buildKIESessionKModuleMemoryFileSystem("spongecell",
						"core-alpha", "0.0.1-SNAPSHOT", "heston-module-alpha",
						"heston-session-alpha");
		validateSession(kieSession1);
	...
	/**
	 * This tests the on-demand use case in which a user requests a new set of
	 * rules to be added to, and executed by, the engine.
	 */
	@Test
	public void validateKieMFSessionHandler() {
		...
		KieSession kieSession1 = kieMFSessionHandler.newBuilder()
				.addGroupId(groupId).addArtifactId(artifactId)
				.addVersion(version).addModelId(moduleId)
				.addSessionId(sessionId).build();
		validateSession(kieSession1);
		...
		KieModule kieModule = kieMFSessionHandler.getModule(groupId,
				artifactId, version, sessionId);
		Assert.assertNotNull(kieModule);
		Assert.assertEquals(kieModule.getReleaseId().getGroupId(), groupId);
		Assert.assertEquals(kieModule.getReleaseId().getArtifactId(),
				artifactId);
